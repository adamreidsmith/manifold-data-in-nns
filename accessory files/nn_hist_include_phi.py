...
of f.  Here, f is given a random phase that is incorporated into training.
...
def main():
    ...
    class Data(Dataset):
        ...
        def __init__(self):
            ...
            self.phase = torch.Tensor([[self.data[i]] for i in range(len(self.data)) if (i+2) % 3 == 0])
    ...
    # item in a loader:
    #   [ [ batch_size number of hists ], [ batch_size number of phi's ], [ batch_size number of [a,b]'s ] ]
    ...
    class Model(nn.Module):
        ...
        def __init__(self):
            ...
            self.fc1 = nn.Linear(in_features=self.in_f+3, out_features=500)
        ...
        def forward(self, x, phi):
            ...
            '''#Incude the phase as a second channel in convolution
            phi_mats = torch.Tensor()
            for p in phi:
                phi_mats = torch.cat((phi_mats,p.item()*torch.ones(100,100).unsqueeze(0)))
            phi_mats.unsqueeze_(1)
            x = torch.cat((x,phi_mats),1)'''
    ...
    #loss_func = lambda x, y: (x-y).pow(4).sum()
    ...
    #Increase the weights corresponding to the phase input neuron so that it has a larger effect         
    #model.fc1.weight.data[:,-1] = model.fc1.weight.data[:,-1]*100
